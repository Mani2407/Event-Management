...
if(isset($_POST['reason_submit'])){
  $name = mysqli_real_escape_string($conn, $_POST['reason']);
      ...
      $updatbooking="update bookings set note='$name',status='0' where id='$booking_id'";
      $runEvents = mysqli_query($conn, $updatbooking);
      if ($runEvents) {
        showErrorSuccessModel(1, 'Booking Rejected.');
        echo "<script>window.location.href='http://localhost/em/admin/requests.php'</script>";
      } else {
